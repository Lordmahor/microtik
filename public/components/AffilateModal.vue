    ...
    <form class="ui mini form" style="padding: 20px">
      <h4 class="ui dividing header">
        <i
            ...
            class="angle icon"
        ></i>
        Information by {{aff.title}}
      </h4>
      <div v-if="information">
        <div class="fields">
          <div class="four wide field">
            <label>Title</label>
            <input placeholder="Login" v-model="data.title">
          </div>
          <div class="four wide field">
            <label>Login</label>
            <input placeholder="Login" v-model="data.login">
          </div>
          <div class="four wide field">
            <label>Pass</label>
            <input placeholder="Password" v-model="data.pass">
          </div>
          <div class="four wide field">
            <label>Wallet</label>
            <input placeholder="Wallet" v-model="data.wallet">
          </div>
          <div class="three wide field">
            <label>Worker</label>
            <div class="ui selection dropdown worker">
              <div class="text"></div>
        ...
        <div @click.prevent="onSubmit" class="ui mini primary button">Submit</div>
        ...
        <div  style="margin-top: 20px">
          ...
          <div class="fields">
            <div class="three wide field">
              <label>FraudCheck</label>
              <div class="ui selection dropdown fraud">
                <div class="text"></div>
                <i class="dropdown icon"></i>
              </div>
            </div>
            <div style="margin-left: 100px" class="four wide field">
              <label :style="[ data.jokerSetting.trafficSave ? 'color: rgb(138, 168, 139)':'color:rgb(168, 138, 138)']"> TrafficSave</label>
              <div style="margin-top: 6px" class="ui slider checkbox savingMode">
                <input
                  type="checkbox"
                  v-model="data.jokerSetting.trafficSave"
                >
              </div>
            </div>
            <div class="four wide field">
              <label :style="[ data.jokerSetting.fingercheck ? 'color: rgb(138, 168, 139)':'color:rgb(168, 138, 138)']"> FingerCheck</label>
              <div style="margin-top: 6px" class="ui slider checkbox savingMode">
                <input
                  type="checkbox"
                  v-model="data.jokerSetting.fingercheck"
                >
              </div>
            </div>
            <div class="four wide field">
              <label :style="[ data.jokerSetting.touchOn ? 'color: rgb(138, 168, 139)':'color:rgb(168, 138, 138)']">TouchOn</label>
              <div style="margin-top: 6px" class="ui slider checkbox savingMode">
                <input
                  type="checkbox"
                  v-model="data.jokerSetting.touchOn"
                >
              </div>
            </div>
          </div>
          <div class="fields">
            <div class="four wide field">
              <label>Link</label>
              <input placeholder="Wallet" v-model="data.link">
            </div>
            <div class="twelve wide field">
              <label>List Country</label>
              <div class="ui fluid multiple search selection dropdown country">
...
export default {
  ...
  watch: {
    aff(v) {
      this.data = {
        ...
        wallet: v.wallet,
        link: v.link,
        jokerSetting: {...v.jokerSetting}
      }
      this.country = this.data.jokerSetting.country
      $('.ui.dropdown.country').dropdown('clear')
      $('.ui.dropdown.country').dropdown('set selected', this.data.jokerSetting.country.split(','))
      let fraudValues = [{
        name: 'none',
        value: 'none',
        selected: false
      ...
      if(fraudValues.find(i => i.value === this.data.jokerSetting.fraudcheck)){
        fraudValues.find(i => i.value === this.data.jokerSetting.fraudcheck).selected = true
      }else {
        fraudValues[3].selected = true
      }
      $('.ui.dropdown.fraud').dropdown({values: fraudValues,
        onChange: (value) => {
          this.data.jokerSetting.fraudcheck = value
        }
      })
      let valuesWorkers = [
          // {name: this.$store.getters.getCurrentUser.email.split('@')[1].split('.')[0], value: 100000000,selected: false},
        ...this.$store.getters.getWorkers.map(i => {
          return {
            name: i.title,
            value: i.id,
            selected: false
      ...
      if(valuesWorkers.find(i => i.value === this.aff.workerId)){
        (valuesWorkers.find(i => i.value === this.aff.workerId)).selected = true
      }else {
        valuesWorkers[0].selected = true
      }
      $('.ui.dropdown.worker').dropdown({
        values: valuesWorkers,
        onChange: (value) => {
          if(this.aff.workerId !== Number(value) && Number(value) >= 100000000) {
            this.data.workerId = Number(value)
          }
        }
      });
      let values = [{name: this.$store.getters.getCurrentUser.email.split('@')[1].split('.')[0], value: 100000000,selected: false},
        ...this.$store.getters.getDrops.map(i => {
          return {
            name: i.title,
            value: i.id,
            selected: false
      ...
      if(values.find(i => i.value === this.aff.dropId)){
        (values.find(i => i.value === this.aff.dropId)).selected = true
      }else {
        values[0].selected = true
      }
      // if(!values.find(i => i.selected === true)) values[0].selected = true
      $('.ui.dropdown.drop').dropdown({values,
        onChange: (value) => {
          if(this.aff.dropId !== Number(value) && Number(value) >= 100000000) {
            this.data.dropId = Number(value)
          }
        }});
    },
  },
  methods: {
    async onSubmit() {
      if(!this.data.workerId) this.data.workerId = 100000000
      if(!this.data.dropId) this.data.dropId = 100000000
      this.data.jokerSetting.country = $('.ui.dropdown.country').dropdown('get value')
      if (confirm(`Вы уверенны ?`)) {
        try{
          await this.$store.dispatch('affilate', this.data)
          this.$store.commit('setAff', this.data)
